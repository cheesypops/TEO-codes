...
/* Volvemos a usar int. Los decimales se truncarán (3.14 -> 3) */
...
void emit(int valor) {
    if (pc >= MAX_CODE_SIZE) {
        fprintf(stderr, "Error CodeGen: Desbordamiento de buffer.\n");
        exit(1);
    }
...
void generar_nodo(ASTNode* nodo) {
    ...
    switch (nodo->tipo) {
        ...
        case NODO_NUMERO: 
            emit(OP_CONST); 
            /* Truncamos a entero explícitamente */
        ...
        case NODO_LLAMADA_FUNCION: {
            if (nodo->hijo1->tipo == NODO_FUNCION_RESERVADA) {
                ...
                else if (strcmp(func, "esperar") == 0) emit(OP_DELAY); /* <--- DELAY incluido */
            }
            break;
        }
        default: break;
...
void generar_codigo(ASTNode* raiz, const char* nombre_archivo) {
    pc = 0;
    printf("\n[CodeGen] Generando Bytecode en memoria...\n");
    generar_nodo(raiz);
    ...
    while (i < pc) {
        ...
        if (args > 0) {
            ...
            fprintf(f, " %d", val); /* <--- Escribe ENTERO */
            i++; 
        }
        fprintf(f, "\n");
        i++;
    ...
    fclose(f);
    printf("[CodeGen] Archivo generado exitosamente.\n");
}